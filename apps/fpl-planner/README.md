...
**"His club has played" means kicked off, and it is counted per club.** FPL
credits `starts` at kickoff and `minutes` while a match is played, so a season
total includes a match in play. The classifier's bound, and every denominator a
player falls back to (`evidence.matchesByClub`), count the matches his own club
has kicked off (`matchesKickedOffByClub` in `engine/lifecycle.js`), allowing the
totals to lead the fixture list by one match because the proxy and the browser
cache the two endpoints for different lengths of time. Comparing against played
out matches instead read every GW4 match window of 2026/27 as last season and
projected a 6.4 point best eleven (FINDINGS, "One live match read the whole
league as last season").
...
load it. The page has no `[data-sync-status-slot]` pill, so the widget renders
into `#sync-banner` alone: the offline notice, a failed or rejected cloud write,
a sync that could not start, and a merge conflict. Until 2026-09-13 the page
carried the banner element and its stylesheet but never loaded the script, so
none of those ever appeared here.
...
every GW4 kickoff window from it (before kickoff, in play, with the fixture list
