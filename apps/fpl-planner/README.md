...
- Its `entry_history.value` is a **snapshot frozen at that gameweek's deadline**, not a live total: it does not move when prices do, so one overnight change is enough to make it disagree with what the squad is actually worth. SQUAD VALUE in the header is therefore the reconstructed total (the fifteen at their selling prices, plus the bank), and a disagreement is reported rather than absorbed. The frozen figure is used for display in one case only: an in-season payload that arrives with no picks, where there is nothing to reconstruct from.
