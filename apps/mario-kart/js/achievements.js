...
function calculateAchievements(player, raceData) {
  ...
  const finalOverallAverage =
    chronologicalRaces.reduce((sum, race) => sum + race[player], 0) / chronologicalRaces.length;
  ...
  achievements.clutchMaster = calculateAchievementLevel(
    maxClutchStreak,
    ACHIEVEMENTS.clutchMaster.targets,
  );
  ...
  achievements.clutchMaster.details.currentActiveStreak =
    currentClutchStreak > 0
      ? {
  ...
  achievements.hotStreak = calculateAchievementLevel(
    maxPodiumStreak,
    ACHIEVEMENTS.hotStreak.targets,
  );
  ...
  achievements.momentumBuilder = calculateAchievementLevel(
    maxMomentumStreak,
    ACHIEVEMENTS.momentumBuilder.targets,
  );
  // Always use max streak for display, but also track current active streak
  achievements.momentumBuilder.details = maxMomentumStreakDetails;
  achievements.momentumBuilder.details.currentActiveStreak =
    currentMomentumStreak > 0
      ? {
...
function toggleAchievementDetails(player, achievementKey) {
  ...
  if (existingExpandedItem) {
    ...
    const mainAchievementBar = document
      .querySelector(`[data-player="${player}"][data-achievement="${achievementKey}"]`)
      .closest('.achievement-bar');
    ...
    const hasActiveStreak = checkForActiveStreak(
      achievementKey,
      achievementData,
      player,
      getFilteredRaces(),
...
function smartUpdateExpandedAchievements(raceData) {
  players.forEach((player) => {
    ...
    const toggleActiveText =
      toggleActiveButton && toggleActiveButton.querySelector('.toggle-active-text');

    const isExpandActiveActive =
      toggleActiveText && toggleActiveText.textContent === 'Collapse Active';
    ...
    Object.keys(ACHIEVEMENTS).forEach((achievementKey) => {
      ...
      const hasActiveStreak =
        hasData && checkForActiveStreak(achievementKey, achievementData, player, raceData);
      ...
      if (existingExpanded && !hasData) {
        ...
        if (detailsElement) {
          detailsElement.innerHTML = generateAchievementDetail(
            achievementKey,
            achievementData,
            player,
          );
...
function togglePositionHeatDetails(player, rangeIndex) {
  ...
  if (existingExpandedItem) {
    ...
    const mainHeatBar = document
      .querySelector(`[data-player="${player}"][data-range="${rangeIndex}"]`)
      .closest('.position-heat-bar');
...
function generateAchievementDetail(achievementKey, achievement, player) {
  ...
  const getStreakStatus = (streakRaces) => {
    ...
    const isActive =
      lastStreakRace.date === lastPlayerRace.date &&
      lastStreakRace.timestamp === lastPlayerRace.timestamp;

    // Find breaking position if streak ended
    let breakingPosition = null;
    if (!isActive) {
      const streakEndIndex = playerRaces.findIndex(
        (race) => race.date === lastStreakRace.date && race.timestamp === lastStreakRace.timestamp,
      );
  ...
  switch (achievementKey) {
    ...
    case 'hotStreak':
      ...
      let podiumResult =
        `${podiumRaceLabel}: ${formatRaceRange(podiumRaceNumbers)}<br>` +
    ...
    case 'momentumBuilder':
      ...
      const activeMomentumCount = hasActiveMomentum
        ? achievement.details.currentActiveStreak.count
        : 0;
      const bestMomentumCount = achievement.details.races.length;
      const showActiveMomentumAsMain =
        hasActiveMomentum && activeMomentumCount >= bestMomentumCount;
    ...
    case 'perfectDay':
      ...
      const threshold = getGoodFinishThreshold();
      const isActiveToday =
        todayRaces.length > 0 && todayRaces.every((race) => race[player] <= threshold);
      ...
      const dayLabel =
        isToday && isActiveToday
          ? `${formatDateTime(bestDay.date)} (Active)`
          : formatDateTime(bestDay.date);
      return (
        `${dayLabel}: total of ${bestDay.raceCount} race${bestDay.raceCount === 1 ? '' : 's'}<br>` +
...
function getRelativeColor(player, achievementValues) {
  ...
  const uniqueNonZeroValues = [
    ...new Set(achievementValues.map((p) => p.value).filter((v) => v > 0)),
  ].sort((a, b) => b - a);
...
function updateAchievements(raceData = null) {
  ...
  players.forEach((player) => {
    ...
    const playerHasNoData = Object.values(achievements).every(
      (achievement) => achievement.current === null,
    );
    if (playerHasNoData) {
    ...
    Object.keys(achievements).forEach((achievementKey) => {
      const numberElement = document.querySelector(
        `[data-player="${player}"][data-achievement="${achievementKey}"]`,
      );
      ...
      if (achievement.current === null) {
        ...
        if (achievementBar) {
          ...
          achievementBar.setAttribute(
            'aria-label',
            `${achievementDef.name}: ${achievementDef.description}`,
          );
      ...
      const isRelativeColoredAchievement = [
        'winStreak',
        'clutchMaster',
        'hotStreak',
        'momentumBuilder',
        'perfectDay',
      ].includes(achievementKey);
      ...
      if (achievementKey === 'winStreak') {
        ...
        if (achievement.level >= 3)
          color = achievementDef.colors[2]; // Gold
...
function checkForActiveStreak(achievementKey, achievement, player, raceData) {
  ...
  switch (achievementKey) {
    ...
    case 'clutchMaster':
      ...
      return (
        achievement.details.currentActiveStreak && achievement.details.currentActiveStreak.count > 0
      );
  ...
  players.forEach((player) => {
    ...
    Array.from(expandedList.children).forEach((expandedItem) => {
      ...
      if (racesInRange.length === 0) {
        ...
        const mainHeatBar = document.querySelector(
          `[data-player="${player}"][data-range="${rangeIndex}"]`,
        );
        if (mainHeatBar) {
          const heatBar = mainHeatBar.closest('.position-heat-bar');
          if (heatBar) {
  ...
  players.forEach((player) => {
    ...
    ranges.forEach((rangeData, index) => {
      const percentageElement = document.querySelector(
        `[data-player="${player}"][data-range="${index}"]`,
      );
        ...
        if (heatBar) {
          if (racesInRangeCount > 0) {
            ...
            if (existingExpandedItem) {
              ...
              if (expandedSection) {
                const expandedList = expandedSection.querySelector('.expanded-position-heat-list');
                const expandedTitle = expandedSection.querySelector(
      ...
      if (count > 0) {
        ...
        if (spotElement) {
          spotElement.setAttribute(
            'aria-label',
            `Position ${i}: Finished ${count} ${raceText} (${percentage}%)`,
          );
        }
      } else {
        glowElement.style.opacity = '0';
