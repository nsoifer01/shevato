...
export function initPlayerIconDisplay() {
...
function createIconElement(playerKey, playerName) {
  const symbol = window.PlayerSymbolManager
    ? window.PlayerSymbolManager.getSymbol(playerKey)
    : null;
...
export function updateAllPlayerIcons() {
...
function updateOtherPlayerDisplays() {
  ...
  const analysisPlayerNames = document.querySelectorAll(
    '.worst-day-item > span:first-child, .best-day-item > span:first-child, .comeback-item > span:first-child',
  );
...
function handleIconChange(playerKey) {
  ...
  const playerName = window.PlayerNameManager ? window.PlayerNameManager.get(playerKey) : playerKey;
  const symbol = window.PlayerSymbolManager
    ? window.PlayerSymbolManager.getSymbol(playerKey)
    : null;
...
function observeDOMChanges() {
  const observer = new MutationObserver((mutations) => {
    ...
    const relevantChange = mutations.some((mutation) => {
      ...
      if (
        mutation.target.closest('.icon-picker') ||
