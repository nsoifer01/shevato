...
function createTrendCharts(raceData = null) {
  ...
  const dateRange =
    raceData.length > 0 ? `${raceData[0].date} - ${raceData[raceData.length - 1].date}` : 'No data';
        ...
        <div class="analysis-container">
            <div class="analysis-card worst-racing-card">
                ...
                <div class="worst-day-list">
                    ${Object.entries(worstRacingDays)
                      .map(([player, data]) => {
...
function calculateComebackAnalysis(raceData) {
  ...
  players.forEach((player) => {
    ...
    for (let i = 1; i < playerRaces.length; i++) {
      ...
      if (
        prevPosition >= (window.getGoodFinishThreshold ? window.getGoodFinishThreshold() + 1 : 13)
      ) {
        badPositions++;
...
function generatePatternAnalysis(raceData) {
  ...
  if (allPositions.length > 0) {
    ...
    if (ranges.length > 0) {
      ...
      const rangeText =
        mostFrequentRange.start === mostFrequentRange.end
