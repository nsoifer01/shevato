  ...
  state.players.forEach((player) => {
    ...
    state.players.forEach((opponent) => {
      if (player !== opponent) {
        ...
        stats.h2hBiggestWins[player][opponent] = {
          gap: 0,
          raceIndex: -1,
          date: null,
          winnerPosition: null,
        ...
        stats.h2hBiggestDailyWins[player][opponent] = {
          dayWins: 0,
          dayLosses: 0,
          margin: 0,
          date: null,
        ...
        if (race[player1] !== null && race[player2] !== null) {
          ...
          if (race[player1] < race[player2]) {
            ...
            if (currentStreaks[player1][player2] > stats.h2hLongestStreaks[player1][player2]) {
              ...
              stats.h2hStreakDetails[player1][player2] = [
            ...
            if (currentStreaks[player2][player1] > stats.h2hLongestStreaks[player2][player1]) {
              ...
              stats.h2hStreakDetails[player2][player1] = [
                ...currentStreakDetails[player2][player1],
              ];
            }
          }
      ...
      const cells = state.players
        .map((colPlayer) => {
          if (rowPlayer === colPlayer) {
            ...
            const cssClass =
              wins > losses ? 'h2h-winner-text' : wins < losses ? 'h2h-loser-text' : 'h2h-tie';
            ...
            if (displayStreak > 0) {
              ...
              const detailsText =
                streakDetails.length > 0 && !isActiveStreak ? `: ${streakDetails.join(', ')}` : '';
      ...
      const cells = state.players
        .map((colPlayer) => {
          if (rowPlayer === colPlayer) {
            ...
            const totalH2HGames =
              (stats.h2h[rowPlayer][colPlayer] || 0) + (stats.h2h[colPlayer][rowPlayer] || 0);
