...
class TooltipManager {
  ...
  init() {
    ...
    document.addEventListener('mousemove', (event) => {
      if (this.currentTarget && this.tooltipElement.classList.contains('visible')) {
        ...
        if (
          !elementAtPoint ||
          (!this.currentTarget.contains(elementAtPoint) && elementAtPoint !== this.currentTarget)
        ) {
          // Mouse is not over the tooltip element anymore
          const tooltipTarget = event.target.closest(
            '[data-tooltip], [title], [aria-label], button, .achievement-bar, .position-heat-bar, .player-name-label, .sidebar-player-initial, th[onclick]',
          );
  ...
  getDefaultTooltip(element) {
    ...
    const tooltipMap = {
      ...
      '.player-name-label': element.closest('#achievements-container')
        ? null
        : `${element.textContent} - Click to customize icon`,
