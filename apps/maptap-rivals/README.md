...
A game belongs to a rival, so every cross-rival view reads the game log through one `liveGames()` selector and a game whose `rivalId` is no longer there counts nowhere. History surfaces those games in a "games without a rival" notice with Reassign and Delete rather than counting them silently. That state is reachable because cross-device sync carries the rival list and the game log as two independent keys: deleting a rival on one device removes both locally, but another device can still push its own copy of the game log afterwards and win the last-write, leaving games behind an id nobody owns.
...
| WhatsApp import | Import paired games from a WhatsApp chat `.txt` export by mapping chat senders to rivals, with a preview before committing. Every export shape parses: Android 24h and 12h (including the U+202F narrow space before AM/PM), the iOS `[20/08/2026, 21:05:10]` bracket form, and DD/MM locales. Day/month order is inferred from the file (any number above 12 settles it) and, when a file settles neither, US order is assumed with a checkbox to flip it. |
| Export / import / clear | Download a JSON backup, import one, or clear all logged games (rivals and settings are kept). An import is validated before anything is written: unreadable entries are skipped with reasons, repairable ones (missing ids, numeric strings) are fixed with a disclosure, and the result reads "Imported N rivals, M games. Skipped K invalid entries: ...". A file where nothing survives leaves the current data untouched. |
...
A third suite, `tests/app-helpers.test.js`, reaches inside `js/app.js` itself: the IIFE ends with a `window._testExports` block exposing its pure helpers (`classifyContinent` and its bounding boxes, the per-rival `continentBreakdown`, the matrix cell builders, `rivalSummary`, `upsertPastedGame` (the per-rival/per-date save guard), `sanitizeBackup` and the `liveGames`/`orphanGames` selectors, the date helpers `localISO`/`addDaysISO`/`isValidISODate`, the `buildHeatmapWeeks` walk, `parseWhatsAppText`/`dayBucketDate`, and `rivalNameHint`), and the suite loads the file with `node:vm` into a stubbed-browser sandbox (never-settling `fetch`, Map-backed `localStorage` seeded per test, `init()` never runs); the timezone tests re-load it under `TZ=Asia/Jerusalem` and `Pacific/Kiritimati`. The rest of app.js (the paste panel DOM flow, the import/export DOM plumbing, sync, rendering) is covered by the browser suite below and the `.features/` plans.

Browser-level regressions for the 2026-08-22 audit live in `e2e/audit-2026-08.mjs`, in the shape the other apps use (raw CDP, real coordinate clicks, seeded `localStorage`): the real `#import-file` and `#wa-import-file` inputs with fixtures, orphan handling in History, mobile geometry for the matrix and heatmap at 390, focus trap and restore, seeded axe scans (zero serious or critical) over all six views at 1280 and 390, and a two-tab check that a save in one tab reaches the other and a delete is not resurrected. It runs inside `npm run test:browser`.
