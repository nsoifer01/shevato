// Pagination Manager for Game History Table
const PaginationManager = (function() {
    ...
    function init() {
        currentPage = 1;
        const savedPageSize = localStorage.getItem('gameHistoryPageSize');
    ...
    function getTotalPages() {
        return Math.ceil(totalGames / rowsPerPage);
    }
    
    // Get paginated subset of games (keeping the same function name for compatibility)
    function getPaginatedRaces(games) {
        totalGames = games.length;
        const startIndex = (currentPage - 1) * rowsPerPage;
        const endIndex = startIndex + rowsPerPage;
        paginatedGames = games.slice(startIndex, endIndex);
    ...
    function setRowsPerPage(newSize) {
        if (pageSizeOptions.includes(newSize)) {
            ...
            localStorage.setItem('gameHistoryPageSize', newSize);
            if (window.updateUI) {
                window.updateUI();
            }
        }
    ...
    function createPaginationControls() {
        ...
        <div class="pagination-container">
            <div class="pagination-info">
                Showing ${((currentPage - 1) * rowsPerPage) + 1}-${Math.min(currentPage * rowsPerPage, totalGames)} of ${totalGames} games
            </div>
