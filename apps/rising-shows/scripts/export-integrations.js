...
// Read apps/rising-shows/data.json and write the static Kometa/MDBList
...
// /apps/rising-shows/kometa/ (which re-runs integrations-lib.js client-side).
//
// Tunables (env vars):
...
(function main() {
  ...
  for (const c of collections) {
    ...
    console.log(`  kometa/${c.filename.padEnd(30)} ${c.seriesCount.toLocaleString().padStart(6)} IDs of ${c.qualifyingCount.toLocaleString()} qualifying series`);
  }
  ...
  const readmeLines = [];
  readmeLines.push('# Rising Shows - NAS integration exports');
  readmeLines.push('');
  readmeLines.push('Generated artifacts consumed by Kometa, MDBList, and Plex.');
  readmeLines.push('Do not edit by hand: regenerated by `npm run export:rising-shows`.');
  readmeLines.push('');
  ...
  for (const c of collections) {
    readmeLines.push(`      - file: config/rising-shows/${c.filename}`);
  }
  readmeLines.push('    overlay_files:');
  readmeLines.push('      - file: config/rising-shows/season-overlays.yml');
  readmeLines.push('```');
  readmeLines.push('');
  // `IDs`, not `Series`: the number counts what the file actually lists.
  // Series with no TMDB/TVDB id are dropped, and distinct IMDb series that
  // share one TMDB id collapse onto a single entry, so the qualifying-series
  // total is larger than the file (rebound: 4,225 qualify, 3,985 IDs).
  ...
  readmeLines.push('');
  readmeLines.push('A point-and-click builder lives at `/apps/rising-shows/kometa/`:');
