...
const SHAPE_META = {
  'rising':              { title: 'Rising Shows',              badge: 'RISE',   blurb: 'Seasons whose episode ratings never dip - every episode meets or exceeds the previous one.' },
...
// Render a YAML scalar safely. Kometa configs are plain YAML, so strings that
...
function buildKometaCollections(matches, opts = {}) {
  ...
  for (let i = 0; i < COLLECTION_SHAPES.length; i++) {
    ...
    // Two reasons the emitted list is shorter than `candidates`, both of which
    // used to be invisible in the header count (rebound claimed 4,225 and
    // listed 3,985):
    //  - a series carrying neither a TMDB nor a TVDB id has nothing Kometa can
    //    match on and is dropped;
    //  - 458 TMDB ids are shared by more than one IMDb series in data.json
    //    (tmdb 1920 is both Twin Peaks entries), so distinct series collapse
    //    onto one id. Kometa dedupes builders anyway; emitting the duplicate
    //    only inflated the count.
    // Count what is emitted, so the header, the README table and the file agree.
    const tmdbIds = [];
    const tvdbIds = [];
    ...
    for (const c of candidates) {
      if (c.tmdbId) {
        ...
        tmdbIds.push(c.tmdbId);
      } else if (c.tvdbId) {
        if (seenTvdb.has(c.tvdbId)) continue;
        seenTvdb.add(c.tvdbId);
        tvdbIds.push(c.tvdbId);
      }
    }
    const emittedCount = tmdbIds.length + tvdbIds.length;
    if (emittedCount === 0) continue;

    const sortTitle = `!${String(i + 1).padStart(2, '0')}_${sortPrefix}_${shape}`;
    const lines = [];
    lines.push(`# Generated by Rising Shows (apps/rising-shows) - do not edit by hand.`);
    lines.push(`# Source: https://shevato.com/apps/rising-shows/  •  Shape: ${shape}`);
    lines.push(`# Floor: confidence >= ${floor}.  Series count: ${emittedCount}.`);
...
function buildSeasonOverlays(matches, opts = {}) {
  ...
  const lines = [];
  lines.push(`# Generated by Rising Shows - season-level shape badges.`);
  ...
  lines.push(`#     - file: config/rising_shows_overlays.yml`);
  lines.push(``);
  lines.push(`overlays:`);
...
const FINDER_BASE_URL = 'https://shevato.com/apps/rising-shows/';
...
function buildFinderCollection(preset, rows, info = {}) {
  ...
  summaryParts.push(`Explore on Rising Shows: ${finderShareUrl(preset.query)}`);
...
// Collection title for a hand-picked Compare set. Two shows read as a
// head-to-head, more than two get the first two plus a count so the name stays
// short enough for Plex's collection grid. Long titles are clipped rather than
// wrapped, since the point is recognising the set at a glance.
function compareCollectionName(titles) {
  const clip = (t) => (t.length > 28 ? t.slice(0, 27).trimEnd() + '…' : t);
  if (titles.length === 0) return '';
  if (titles.length === 1) return clip(titles[0]);
  if (titles.length === 2) return `${clip(titles[0])} vs ${clip(titles[1])}`;
  return `${clip(titles[0])}, ${clip(titles[1])} + ${titles.length - 2} more`;
}

// Kometa collection YAML for the Compare set the user assembled in the browser
// (Compare modal in index.html). `shows` are {seriesId, title, tmdbId, tvdbId}
// records in the order they were added. A show carrying neither a TMDB nor a
// TVDB id is skipped - Kometa has nothing to match it on - and the collection
// is named after the titles that survive, so it is identifiable in Plex without
// renaming. The summary links back to the compare permalink built from the
// FULL set (that link is about the on-site comparison, not the Plex payload).
// Returns null when no show has a usable id.
function buildCompareCollection(shows) {
  const all = Array.isArray(shows) ? shows.filter(Boolean) : [];
  const rows = all.filter((s) => s.tmdbId || s.tvdbId);
  if (rows.length === 0) return null;

  const name = compareCollectionName(rows.map((r) => r.title || 'Untitled'));
  const tmdbIds = [];
  const tvdbIds = [];
  for (const r of rows) {
    if (r.tmdbId) tmdbIds.push(r.tmdbId);
    else tvdbIds.push(r.tvdbId);
  }
  const permalinkIds = all.map((s) => s.seriesId).filter(Boolean);

  const summaryParts = ['Hand-picked comparison from Rising Shows.'];
  if (permalinkIds.length) {
    summaryParts.push(`Compare on Rising Shows: ${finderShareUrl(`compare=${permalinkIds.join(',')}`)}`);
  }

  const lines = [];
  lines.push(`# Generated by Rising Shows (apps/rising-shows) - do not edit by hand.`);
  lines.push(`# Hand-picked Compare set. Shows: ${rows.length}.`);
  lines.push(`collections:`);
  lines.push(`  ${yamlString(name)}:`);
  lines.push(`    summary: ${yamlString(summaryParts.join('\n'))}`);
  lines.push(`    sort_title: ${yamlString('!000_rsc_compare')}`);
  lines.push(`    collection_order: alpha`);
  lines.push(`    sync_mode: sync`);
  if (tmdbIds.length) {
    lines.push(`    tmdb_show:`);
  ...
  return {
    name,
    filename: 'rising-shows-compare.yml',
    seriesCount: rows.length,
    contents: lines.join('\n') + '\n',
  };
}
...
const API = {
  ...
  buildSeasonOverlays,
  buildIdLists,
  // The single definition of a season's fit for a shape, including the 1.0 for
  // categorical tags. scripts/watch-next.js consumes it for the same reason the
  // builders do: anything reading m.confidence[shape] directly scores
  // saved-best-for-last and shape-drift as 0 and silently loses them.
  shapeConfidence,
  // Exposed for tests:
  yamlString,
