/* Rising Shows → Plex + Kometa builder UI.
   Loads data-index.json (the same slim index the Show Finder fetches at boot,
   so arriving from the Finder revalidates an already-warm cache entry instead
   of downloading a second copy), and calls into
   window.RisingShowsIntegrations to build YAML / ID-list output on the fly as
...
(function () {
  ...
  loadDataset().then(() => {
    ...
    // A clean clone has no data-index.json (it is a build artifact, gitignored
    // and produced by scripts/split-data.js), so say that instead of printing a
    // bare HTTP 404 and leaving the "Loading dataset…" line up forever.
    setStatusError(err && err.missingDataset
      ? 'Dataset not built. data-index.json is generated by the site build '
        + '(npm run build:site, or build:rising-shows:split with data.json '
        + 'already downloaded). On shevato.com it is always present.'
