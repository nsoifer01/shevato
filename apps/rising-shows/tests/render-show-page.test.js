...
test('renderShowPage sets a canonical URL using the slug + tconst', () => {
  const html = renderShowPage(BREAKING_BAD);
  assert.ok(html.includes('<link rel="canonical" href="https://shevato.com/apps/rising-shows/shows/breaking-bad-tt0903747/">'));
});
...
test('renderShowPage links to the IMDb canonical and the shape-filtered explorer', () => {
  ...
  assert.ok(html.includes('/apps/rising-shows/#shape=rising'));
});
...
test('renderShowPage header contains brand link and launch-app button pointing to /apps/rising-shows/', () => {
  const html = renderShowPage(BREAKING_BAD);
  assert.ok(html.includes('class="brand"'));
  assert.ok(html.includes('href="/apps/rising-shows/" aria-label="Rising Shows home"'));
  ...
  assert.ok(snippet.includes('href="/apps/rising-shows/"'));
});
...
test('renderShowPage hero-actions has three buttons: shape CTA, app-btn, IMDb link', () => {
  ...
  assert.ok(appBtnSnippet.includes(`href="/apps/rising-shows/#show=${BREAKING_BAD.seriesId}"`));
  assert.ok(appBtnSnippet.includes('Open in Rising Shows app'));
...
test('renderShowPage app-btn is present even when show has no dominant shape', () => {
  ...
  assert.ok(snippet.includes(`href="/apps/rising-shows/#show=${BREAKING_BAD.seriesId}"`));
});
...
test('buildTvSeasonSchema emits correct ratingValue and ratingCount', () => {
  ...
  const canonical = 'https://shevato.com/apps/rising-shows/shows/breaking-bad-tt0903747/';
