...
| Modal action row         | Each modal has **one** action row, in the heading beside the poster: the primary action first (accent-filled - "+ Add to compare" on the show modal, "Mark as watched" on the season modal), then the ghost utilities (Share card, Share chart image, Permalink, IMDb, TVDB). They wrap to two rows on desktop and three on a phone, each button at least 44 px tall. Between 2026-08-23 and 2026-09-07 the utilities lived in a separate row after the content instead, which put them ~1,900 px down a scrolling panel on desktop and at the very bottom of a ~2,500 px sheet on a phone; see FINDINGS.md for why that was reversed. |
