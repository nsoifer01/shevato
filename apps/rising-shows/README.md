...
|---|---|---|
| `shows-index.json` | **the boot payload.** One record per SHOW, already aggregated by `buildShowAgg`: 34,772 rows, 16.7 MB raw / 3.5 MB brotli at Netlify's q3 on the 2026-09-08 dataset | preloaded from `<head>`; the only thing first paint waits for, and the ONLY dataset fetch at boot |
...
| ------------------------ | -------------------------------------------------------------------------------------------------- |
| A-Z bucketing            | The A-Z index files a title under the letter it sorts by, folding diacritics for bucketing only (never for display). Latin letters that are their own base character rather than a letter plus a mark (the AE and OE ligatures, slashed O, crossed L, eth, thorn, sharp s) are named explicitly, because NFKD leaves them alone and they otherwise land in "#" beside the digits. Genuinely non-Latin scripts do stay in "#": transliterating them would be a guess about a romanisation the reader may not share. |
...
| Loading state            | The index is a ~16 MB JSON file (~3.5 MB brotli over the wire), so the first paint is skeleton cards plus a live status in the result-count line ("Loading show index (6 of 16 MB)..."), which is the page's `aria-live="polite"` region. `index.html` preloads it from `<head>`, so the download no longer waits for the page to parse and the deferred scripts to run (see FINDINGS.md, "The index download starts in the head"). The search box stays usable and is marked `aria-busy`: **anything typed while the index downloads is kept**, merged into the finder state when the data lands, and written to the hash (a URL that already carries `q=` wins). A malformed index (not just a 404 or truncated JSON) reaches the same "Couldn't load show data - Retry" panel instead of leaving the skeletons up: `validateDataset` drops records missing an id/title/season and errors when nothing usable is left. |
...
| Best / worst / most-rated badges | Inline markers on the highest- and lowest-rated season of a show, and on the highest-, lowest- and most-rated episode inside a season. Best and worst are skipped when there is nothing to compare (a single season, a single episode) or when everything ties, because a badge that names the same entry best and worst says nothing. Most rated is popularity rather than quality, so it is an independent badge and often lands on the best season or episode as well. Ties keep the earlier season or episode. A season's rating count is the sum of its episodes' votes, folded when the modal opens (the index carries only `minVotes`, the season's floor, not a total), so a show whose detail fetch failed keeps its best/worst badges - those come from index-backed averages - and simply shows no most-rated one. Both levels go through one helper, `pickHighlights`. |
...
| Above-IMDb badge         | Marks seasons whose average episode rating beats the show's overall IMDb score, **only for shows with at least 1,000 IMDb votes** (`ABOVE_IMDB_MIN_VOTES`): below that a handful of fans rating every episode 10.0 produces the badge trivially, which is a claim the data cannot support. The show-level pill in the modal reads the same precomputed `aboveImdb` list the grid does, so the two surfaces cannot disagree. |
| One average, everywhere  | "Avg episode" is the episode-weighted mean over every rated episode (`sum(ratings) / count`), rounded to 2 dp, on the card, in the list row, in the modal, in the share card and on the share image, and the same fold decides the Above-IMDb verdict everywhere (all surfaces read one precomputed list). The static pages print that same fold at 1 dp in three places: the hero's "Episode average" stat (with its signed delta against the IMDb rating directly above it), `og:image:alt` and `twitter:data2`. The hero row asserts the "beats IMDb" mark on exactly the app's terms (`renderEpisodeAverageRow` mirrors `aboveImdbBadge`: the average must beat the series rating AND the show must clear `ABOVE_IMDB_MIN_VOTES`), so a thinly voted show never gets the arrow on either surface; every other delta prints muted, with no claim. Verified over the whole catalogue: 32,325 pages print it and every one agrees with the app. That needed the arithmetic to be order-independent as well as identical, since ratings carry one decimal and a show whose mean lands exactly on a .005 boundary (The Boys, 327.4 across 40 episodes) rounds either way depending on how the sum was accumulated; all three folds go through integer tenths. The modal used to compute an unweighted mean of the per-season averages instead, which differed at 1 dp for 2,784 multi-season shows (Master of None: 7.97 on its card, 7.6 in its modal) and flipped the Above-IMDb verdict for 162 of them. |
...
| Share chart image        | A "Share chart image" button next to "Share card" in the show modal, and in the compare overlay, composites the on-screen SVG curve plus the title, dominant shape and key stats onto an offscreen canvas and hands back a PNG (1200px wide). It **copies the image to the clipboard** so you can paste it straight into a message, the way "Share card" beside it copies text; where an image clipboard write is not available (iOS Safari, no secure context, an unfocused document) it falls back to the native share sheet, and to a plain download only if neither works. The button says which one happened - "Copied!", "Shared!" or "Downloaded!" - the way the existing copy buttons flash "Copied!". Everything is same-origin so the canvas is never tainted. The show-modal button is hidden for single-season shows, the same gate the overlay chart itself uses; the compare variant captures every legend entry in the colors `renderCompareLegend` assigned. |
...
| `tests/split-data.test.js` | the payload split: that the emitted `data-index.json` aggregates identically to the unsplit `data.json` through the real `buildShowAgg`, that index + detail rehydrates losslessly, the `aboveImdb` rules, the still-airing flag reaching the show index on the right season only, and the `<head>` preload matching the URL `load()` fetches in the mode it fetches it |
| `tests/shows-index-parity.test.js` | that `shows-index.json` equals the pre-split boot fold field-for-field, over a synthetic catalogue (always) and the real one (when the release data is present): the aggregate, the season averages, the sparkline ratings, and the four answers that used to be scanned out of season records at boot (above-IMDb, provider chip, adult flag, best/worst season). Then the observable answers: the whole Finder pipeline (`filterAndSortRows`) over 78 filter / sort / search states including 40 seeded-random combinations, `computeShowRelated` over a 400-show spread, the suggestion index over the whole catalogue, and that the show index flags exactly the still-airing seasons the season file does |
...
| `tests/app-features.test.js` | browser helpers reached through a `node:vm` sandbox (see below): the canonical weighted `avgEpisode`, diacritic folding, the related-show gates and ranking, the NaN guards, the scroll/watched/compare stores, the index schema guard, the episode-count fallback, the shape-confidence read, the still-airing season and its label, and the search suggestions: the Sorensen-Dice picks, strict ranking and ties, no row walk per keystroke, and a reloaded catalogue never searched through the old index |
...
| `tests/classic-script-globals.test.js` | executes the three classic libraries index.html loads (match, finder-lib, providers-lib) in one shared context and checks each namespace reaches `window`. The name-clash check for every published page, this one included, is `tests/static/classic-script-scope.test.mjs` (see FINDINGS) |
| `tests/fetch-data.test.js` | the build-time dataset download: the committed `data-release.json` pin, release-stamped asset names, SHA-256 verification of both files before either reaches disk, and a half-present pair treated as absent |
| `tests/modal-actions.test.js` | the modal action row: where the share / permalink / IMDb / TVDB controls live in the markup, and that the CSS hierarchy rules are scoped to the class that actually wraps them |
| `tests/providers-lib.test.js` | the shared streaming vocabulary: plan and channel variants collapse to a brand, the mainstream set, and that index.html loads it before app.js |
| `tests/validate-dataset.test.js` | the shipped-dataset validator itself: one deliberately wrong dataset per check, plus a healthy control |
