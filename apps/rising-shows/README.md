...
| Modal action row         | Each modal has **one** action row, in the heading beside the poster, in three tiers. The primary action holds its own line (accent-filled: "+ Add to compare" on the show modal, "Mark as watched" on the season modal). Below it the ghost action buttons (Share card, Share chart image, Permalink), each at least 44 px tall on a phone. Ending that line, the outbound links (IMDb, TVDB) are compact pills with a ↗ mark rather than buttons: they leave the site, and as full buttons they cost a whole extra line and made five different things look like five equal choices. A pill shows only the site name, so what it points at ("Season 1 on IMDb", "This series on TVDB") is set as its accessible name and tooltip, always containing the visible word. Between 2026-08-23 and 2026-09-07 the utilities lived in a separate row after the content instead, which put them ~1,900 px down a scrolling panel on desktop and at the very bottom of a ~2,500 px sheet on a phone; see FINDINGS.md for why that was reversed. |
