...
(its last rated episode is not the last episode IMDb lists for it, or it is a
current-year season much shorter than the one before) carries `inProgress` and
...
747 of 66,380 seasons carry the flag on the 2026-08-22 build. See FINDINGS.md
...
| One average, everywhere  | "Avg episode" is the episode-weighted mean over every rated episode (`sum(ratings) / count`), rounded to 2 dp, on the card, in the list row, in the modal, in the share card, on the share image and on the static page. The modal used to compute an unweighted mean of the per-season averages instead, which differed at 1 dp for 2,784 multi-season shows (Master of None: 7.97 on its card, 7.6 in its modal) and flipped the Above-IMDb verdict for 162 of them. |
