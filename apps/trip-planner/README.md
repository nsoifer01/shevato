...
| Settings | 12/24-hour time format (saved and synced); the trip menu's actions are grouped under Trip, Import & export and Preferences captions, with Delete trip standing on its own at the bottom; Escape closes one layer at a time, topmost first (an open dialog, then the assistant panel, then the search panel, then the trip menu), so a popover can no longer be left stranded open underneath a dialog; a small "build N" tag at the bottom of the trip menu identifies the loaded code version (staleness diagnostics for the PWA cache) |
