...
| Settle up | With two or more named travelers, each item's "Who's this for" section also asks who actually paid ("Paid by", defaulting to a blank "Not tracked"). A "Settle up" block under the per-traveler totals nets every Booked, convertible, payer-tagged cost down to the fewest possible payments and states them as "{debtor} owes {creditor} {amount}", so two people who each covered different bookings see one line, not two. Amounts are rounded to cents before pairing, so a stray sub-cent line can never appear. Nothing tagged yet reads "Add \"Paid by\" to a cost to see who owes whom" rather than a row of misleading zeroes, and everything squaring up reads "All settled up". A payer is re-spelled or dropped when the roster is edited, so a name that left the trip never lingers |
| Cost by type | A "Cost by type" block under the totals bar breaks the trip's Booked, costed items down per type (Flight, Stay, Transport, Local travel, Activity, Note), biggest first, in the display currency. A type with nothing booked and costed gets no row at all rather than a $0.00 placeholder, and a row holding money that could not be converted is flagged amber with "+ N not converted", the same honesty rule the Confirmed total follows. It is gated on cost data alone, so it shows on a solo trip too |
...
| Settings | 12/24-hour time format (saved and synced); the trip menu's actions are grouped under Trip, Import & export and Preferences captions, with Delete trip standing on its own at the bottom; a small "build N" tag at the bottom of the trip menu identifies the loaded code version (staleness diagnostics for the PWA cache) |
