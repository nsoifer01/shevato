...
| Import / export | JSON per trip, full backup of all trips, CSV export (includes per-item currency + converted columns, plus confirmation and travelers columns appended so older spreadsheets keep their column order); import accepts trip files or full backups and re-normalizes traveler names against the trip roster. Attached documents are never included in exports |
...
| Share link | Share itinerary produces a URL with the whole trip compressed into the fragment (no server, payload slimmed of empty fields, including confirmation codes and the traveler roster and per-item split); opens read-only with a banner and "Import as my trip", re-normalizing traveler names on import. Links over ~8k chars copy with a truncation warning; a 30k hard cap points to JSON export |
