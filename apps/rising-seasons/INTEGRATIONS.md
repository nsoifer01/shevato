...
Five ways Rising Seasons plugs into a Plex + Kometa stack. The
exports under `apps/rising-seasons/exports/` are produced by
`npm run export:rising-seasons` and committed alongside `data.json` (the
...
Finder filter, defined in `apps/rising-seasons/finder-presets.json`. Unlike
...
4. Run `npm run export:rising-seasons` and commit
