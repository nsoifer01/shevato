...
class GymTrackerApp {
  ...
  loadAllData() {
    try {
      ...
      this.programs = Array.isArray(programsData)
        ? programsData.map((p) => Program.fromJSON(p))
        : [];
      ...
      this.workoutSessions = Array.isArray(sessionsData)
        ? sessionsData.map((s) => WorkoutSession.fromJSON(s))
        : [];
      ...
      this.achievements = Array.isArray(achievementsData)
        ? achievementsData.map((a) => Achievement.fromJSON(a))
        : [];
  ...
  setupEventListeners() {
    ...
    document.addEventListener('click', (e) => {
      ...
      const navBtn = e.target.closest(
        '.nav-item[data-view], .nav-link[data-view], .btn-text[data-view], .more-item[data-view]',
      );
      if (!navBtn) {
  ...
  showView(viewName, pushState = true) {
    ...
    setTimeout(() => {
      ...
      const navItems = document.querySelectorAll(
        `.nav-item[data-view="${viewName}"], .nav-link[data-view="${viewName}"]`,
      );
...
window.debugGymTracker = {
  clearLocalStorage: () => {
    ...
    const keys = [
      ...
      'gymTrackerSessions',
      'gymTrackerSettings',
      'gymTrackerAchievements',
      'gymTrackerActiveProgram',
