...
class ProgramsView {
  ...
  renderProgramExercises() {
    ...
    container.innerHTML = this.currentProgram.exercises
      .map(
        (exercise, index) => `
  ...
  openExercisePicker() {
    ...
    const filterExercises = () => {
      this.renderExercisePicker(searchInput.value, categoryFilter.value, equipmentFilter.value);
    };
  ...
  renderExercisePicker(searchTerm = '', category = '', equipment = '') {
    ...
    if (searchTerm) {
      ...
      exercises = exercises.filter(
        (ex) =>
          ex.name.toLowerCase().includes(lower) || ex.muscleGroup.toLowerCase().includes(lower),
      );
    }
