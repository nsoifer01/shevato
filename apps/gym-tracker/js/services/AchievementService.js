...
export class AchievementService {
  ...
  static calculateProgress(achievement, sessions) {
    ...
    switch (reqType) {
      ...
      case 'exercises-completed':
        return AnalyticsService.calculateAchievementProgress(
          'exercises',
          sessions,
          'exercises-completed',
        );
  ...
  static getNewlyUnlocked(oldAchievements, newAchievements) {
    const oldUnlockedIds = new Set(oldAchievements.filter((a) => a.unlocked).map((a) => a.id));

    return newAchievements.filter((a) => a.unlocked && !oldUnlockedIds.has(a.id));
  }
