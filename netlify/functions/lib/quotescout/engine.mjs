...
export function createEngine({ adapters, reserve = async () => true, emitMetric = () => {}, now = Date.now, timeout = 12000, concurrency = 3, cache = new BoundedCache(250, now), enrichment = new BoundedCache(500, now) }) {
  ...
  return async function compare(request, scope, emit = () => {}, reserveCall = reserve) {
    ...
    const final = { type: 'done', requestId, checked: all.filter(p => !['UNAVAILABLE'].includes(p.status) || selected.find(a => a.id === p.provider)?.enabled).length, returned: all.reduce((n,p) => n+p.quotes.length,0), unavailable: all.filter(p => !['OK','ADDITIONAL'].includes(p.status)).length, additional: all.filter(p => p.status === 'ADDITIONAL').length };
