...
Build minutes: the production Netlify project is on the legacy Free plan, 300 build minutes a calendar month (Pacific), and Deploy Previews are OFF on it (`skip_prs`, since 2026-09-14), so only pushes to `master` build. `scripts/netlify-ignore.mjs` is the `ignore` rule prepared for 1 October: it skips a build only when every file changed since the commit production serves is Markdown, a test, the root `.github/` or `eslint.config.mjs`, and it builds on any doubt. It is committed and tested (`tests/static/netlify-ignore.test.mjs`) but not yet named in `netlify.toml`. The measurements, the budget and the activation steps are in the root `FINDINGS.md`, "Netlify build minutes".
