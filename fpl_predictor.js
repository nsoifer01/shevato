...
function optimizeTeam(playerFeatures, budget) {
    console.log("Optimizing team with features:", playerFeatures);
    ...
    for (const player of sortedPlayers) {
        ...
        if (teamStructure[position] > 0 && remainingBudget >= player.now_cost / 10 && !uniquePlayers.has(player.web_name)) {
            ...
            console.log(`Selected ${player.web_name} (${position}), remaining budget: ${remainingBudget}`);
        }

        if (selectedTeam.length >= 11) break;
    }

    console.log("Final selected team:", selectedTeam);
    return selectedTeam;
}
